...
この例から[ カスタムスニペット ](https://experienceleague.adobe.com/en/docs/commerce-on-cloud/user-guide/cdn/custom-vcl-snippets/fastly-vcl-allowlist#add-the-custom-vcl-snippet)を作成する前に、値を確認して、変更が必要かどうかを判断してください。 次に、それぞれのフィールドに各値を入力します。例えば、「`type`」を「タイプ」フィールドに、「`content`」をコンテンツフィールドに入力します。
...
- [管理者](#add-the-custom-vcl-snippet)からカスタム VCL スニペットを追加します。 管理者にアクセスできる場合は、この方法をお勧めします。 （Magento 2 バージョン 1.2.58](fastly-configuration.md#upgrade)以降では[Fastly CDN モジュールが必要です）。
