...
- 新しいAdobe Commerce バージョンとの互換性を保つため、MariaDB （MySQL）、OpenSearch、RabbitMQ、Redis の新しいバージョンで `.magento/services.yaml` ファイルをアップデートします。 Pro プロジェクトの場合、ステージング環境と実稼動環境でサービスをインストールまたは更新するには、Adobe Commerce サポートチケットを送信する必要があります。
